...
//    removeChildElement("sidebar", "layerSwitcherContent");
    ...
    jQuery.get('resources/templates/viewer/search.html', function (template) {
        ...
        //Add tad
        addTab($.i18n('gen-search'), template);
        ...
        $('#Layers').empty();
        $('#Layers').append(
                $("<option></option>").attr("value", "Please Select").text($.i18n("gen-please-select")));
...
var populateLayerFields = function (FieldNames1) {
    ...
    for (var i = 0; i < FieldNames1.length; ++i) {
        ...
        if (_type != "Polygon" && _type != "Line" && _type != "Point")
            $('#LayersFields').append($("<option></option>").attr("value", _type).text(($.i18n(FieldNames1[i].name) == "") ? FieldNames1[i].name : $.i18n(FieldNames1[i].name)));
    }
...
var PopulateLayerFields_Search = function (FieldNames1) {
    ...
    for (var i = 0; i < FieldNames1.length; ++i) {
        ...
        $('#LayersFields').append(
                $("<option></option>").attr("value", FieldNames1[i].field).text(($.i18n(FieldNames1[i].alias) == "") ? FieldNames1[i].alias : $.i18n(FieldNames1[i].alias)));
    }
    if (FieldNames1.length > 0) {
        $('#LayersFields').get(0).selectedIndex = 0;
    }
...
var CreatefeatureRequest = function (selectedSearchType1, selectedFieldType, selectedField, searchText) {
    ...
    switch (selectedSearchType1.toUpperCase()) {
        case "EXACT":
            ...
            {
                jAlert($.i18n("err-wrong-field-type", selectedFieldType), $.i18n("err-error"));
    ...
    if (pos > -1) {
        ...
        if (parseDate == true) {
            ...
            if ((month < 1) || (month > 12))
                ...
                valid = false;
            else if ((month == 1) && (((year % 400) == 0) || ((year % 4) == 0)) && ((year % 100) != 0) && (day > 29))
                valid = false;
        } else {
            valid = false;
        }
        ...
        success: function (data1) {
            ...
            for (var i = 0; i < data1.featureTypes[0].properties.length; ++i) {
                if (!data1.featureTypes[0].properties[i].type.indexOf("gml") >= 0) {
                    ...
                    if (lang == 'cy') {
                        myColumns[ctr] = $.i18n(data1.featureTypes[0].properties[i].name);
                    } else {
            ...
            var result_count = "<div style = 'margin-left: 3em; display: inline-block;'>" + $.i18n("gen-result-count") + ": " + "<span id = 'result_cnt'></span></div>";
            var grid_toolbar = "<div class='gridtoolbar fg-buttonset fg-buttonset-single'>";
            grid_toolbar = grid_toolbar
                    + "<button name='tooglefilter' class='vtip fg-button ui-state-default1 ui-priority-primary1 ui-corner-left ui-corner-right' title='" + $.i18n("gen-search") + "' onclick='toggleFilterToolBar();'><img src='resources/images/viewer/layouts/toggle_search.png' width='16px' height='16px'/></button>";
