 ...
 //setParameter("projectName", CB_Proj)
	...
	public  List<LaSpatialunitLand> search(Long status, Integer claimType, String project,String communeId,String transId,String parcelId,Integer startpos) {
		...
		if(status != null && status != 0){
		 ...
		 hql1 =" Select LD.landid, LD.landno, LC.claimtypeid, LC.claimtype_en, LD.area, la.applicationstatus_en, "+
				 ...
				 "inner Join  la_party_organization LP on PL.partyid = LP.organizationid "+ 
				 "inner join la_ext_transactiondetails TR on PL.transactionid = TR.transactionid "+  
				 "where Pl.isactive=true and LD.workflowstatusid!=6 and LD.isactive=true  and LD.projectnameid = " +project +	"  and "+ strWhereClause; 
	...
	public Integer getTotalrecordByProject(String project) {
		...
		try{
			...
			String hql1 = "SELECT SUM(landid) AS landid FROM (Select count(LD.landid) as landid   from la_spatialunit_land LD  "+ 
					"Inner join la_ext_personlandmapping PL on LD.landid = PL.landid  "+
					"inner Join la_right_claimtype LC on LD.claimtypeid=LC.claimtypeid  "+
					"inner Join la_ext_applicationstatus la on la.applicationstatusid =  LD.applicationstatusid "+ 
					"inner Join la_ext_workflow lf on lf.workflowid =  LD.workflowstatusid "+
			        ...
			        "inner join la_ext_transactiondetails TR on PL.transactionid = TR.transactionid "+ 
					"where Pl.isactive=true and LD.workflowstatusid!=6 and LD.isactive=true and  LD.projectnameid = " +project +" Union "+
					"Select count(LD.landid) as landid "+
					"from la_spatialunit_land LD  "+
					"inner Join la_right_claimtype LC on LD.claimtypeid=LC.claimtypeid "+ 
					"inner Join la_ext_applicationstatus la on la.applicationstatusid =  LD.applicationstatusid "+ 
					"inner Join la_ext_workflow lf on lf.workflowid =  LD.workflowstatusid "+
					"where LD.isactive=true AND LD.claimtypeid=4 and LD.isactive=true and  LD.projectnameid= " +project +" Union "+
					"Select count(LD.landid) as landid   from la_spatialunit_land LD "+   
					"Inner join la_ext_personlandmapping PL on LD.landid = PL.landid  "+
					"inner Join la_right_claimtype LC on LD.claimtypeid=LC.claimtypeid  "+
					"inner Join la_ext_applicationstatus la on la.applicationstatusid =  LD.applicationstatusid "+  
					...
					"inner Join  la_party_organization LP on PL.partyid = LP.organizationid "+ 
	                "inner join la_ext_transactiondetails TR on PL.transactionid = TR.transactionid "+ 
			        "where Pl.isactive=true and LD.workflowstatusid!=6 and LD.isactive=true and  LD.projectnameid = " +project +
					") AS a" ;
			
		
			List<BigDecimal> arrObject = getEntityManager().createNativeQuery(hql1).getResultList();
	...
	public Integer searchCount(Long status, Integer claimType, String project,String communeId,String transId,String parcelId) {
		...
		if(status != null && status != 0){
			...
			hql1 =" SELECT SUM(landid) AS landid FROM ( Select count(LD.landid) as landid  from la_spatialunit_land LD  "+ 
					 "Inner join la_ext_personlandmapping PL on LD.landid = PL.landid  "+
					 "inner Join la_right_claimtype LC on LD.claimtypeid=LC.claimtypeid  "+
					 "inner Join la_ext_applicationstatus la on la.applicationstatusid =  LD.applicationstatusid "+ 
					 "inner Join la_ext_workflow lf on lf.workflowid =  LD.workflowstatusid "+
					 ...
					 "inner Join la_ext_applicationstatus la on la.applicationstatusid =  LD.applicationstatusid "+ 
					 "inner Join la_ext_workflow lf on lf.workflowid =  LD.workflowstatusid "+
					 "where LD.isactive=true AND LD.claimtypeid=4 and LD.isactive=true and  LD.projectnameid= " +project  +" and " +  strWhereClause1 +" Union "+
					 "Select count(LD.landid) as landid   from la_spatialunit_land LD "+   
						"Inner join la_ext_personlandmapping PL on LD.landid = PL.landid  "+
						"inner Join la_right_claimtype LC on LD.claimtypeid=LC.claimtypeid  "+
						"inner Join la_ext_applicationstatus la on la.applicationstatusid =  LD.applicationstatusid "+  
						...
						"inner Join  la_party_organization LP on PL.partyid = LP.organizationid "+ 
		                "inner join la_ext_transactiondetails TR on PL.transactionid = TR.transactionid "+ 
				        "where Pl.isactive=true and LD.workflowstatusid!=6 and LD.isactive=true and  LD.projectnameid="+  project +" and " + strWhereClause + " ) AS a" ;
	...
	public Integer spatialUnitWorkflowCount(int[] workflow_ids,int[] claim_ids,int[] status_ids, String project) {
        ...
        if(!resultwrk.isEmpty()){
		 ...
		 if(!resultsts.isEmpty()){
			...
			/* hql1 = "Select count (LD.landid)  from la_spatialunit_land LD  "+ 
						"Inner join la_ext_personlandmapping PL on LD.landid = PL.landid  "+
						"inner Join la_right_claimtype LC on LD.claimtypeid=LC.claimtypeid  "+
						"inner Join la_ext_applicationstatus la on la.applicationstatusid =  LD.applicationstatusid "+ 
						"inner Join la_ext_workflow lf on lf.workflowid =  LD.workflowstatusid "+
	...
	/*	 hql1 = " SELECT SUM(landid) AS landid FROM (Select count (DISTINCT LD.landid) as landid  from la_spatialunit_land LD  "+ 
					"Inner join la_ext_personlandmapping PL on LD.landid = PL.landid  "+
					"inner Join la_right_claimtype LC on LD.claimtypeid=LC.claimtypeid  "+
					"inner Join la_ext_applicationstatus la on la.applicationstatusid =  LD.applicationstatusid "+ 
					"inner Join la_ext_workflow lf on lf.workflowid =  LD.workflowstatusid "+
	               ...
	               " Select count (DISTINCT LD.landid) as landid   from la_spatialunit_land LD  "+ 
					"Inner join la_ext_disputelandmapping PL on LD.landid = PL.landid  "+
					"inner Join la_right_claimtype LC on LD.claimtypeid=LC.claimtypeid  "+
					"inner Join la_ext_applicationstatus la on la.applicationstatusid =  LD.applicationstatusid "+ 
					"inner Join la_ext_workflow lf on lf.workflowid =  LD.workflowstatusid "+
					"inner Join  la_party_person LP on PL.partyid = LP.personid " +
	                " inner join la_ext_transactiondetails TR on PL.transactionid = TR.transactionid "+ 
	                " where  Pl.isactive=true and LD.workflowstatusid!=6 and LD.isactive=true and  LD.projectnameid = " +project +" and " + strWhereCls +"  ) AS a";*/
		 ...
		 hql1 = " SELECT SUM(landid) AS landid FROM (Select count (DISTINCT LD.landid) as landid  from la_spatialunit_land LD  "+ 
	                ...
	                "inner Join la_ext_applicationstatus la on la.applicationstatusid =  LD.applicationstatusid "+ 
	                "inner Join la_ext_workflow lf on lf.workflowid =  LD.workflowstatusid "+
	                "where LD.isactive=true AND LD.claimtypeid=4 and LD.isactive=true and  LD.projectnameid  = " +project +" and " + strWhereCls +" Union "+
	                "Select count (DISTINCT LD.landid) as landid   from la_spatialunit_land LD  "+ 
					"Inner join la_ext_disputelandmapping PL on LD.landid = PL.landid  "+
					"inner Join la_right_claimtype LC on LD.claimtypeid=LC.claimtypeid  "+
					"inner Join la_ext_applicationstatus la on la.applicationstatusid =  LD.applicationstatusid "+ 
	...
	public List<LaSpatialunitLand> getspatialUnitWorkFlowResult(int[] workflow_ids,int[] claim_ids,int[] status_ids, Integer startfrom, String project) {
		...
		try {
	    		...
	    		/* hql1 = "Select LD.landid, LD.landno, LC.claimtypeid, LC.claimtype_en, LD.area, la.applicationstatus_en, "+
						" LP.firstname, LP.lastname, LP.address,la.applicationstatusid ,LD.workflowstatusid ,lf.workflow_en   ,TR.transactionid from la_spatialunit_land LD  "+ 
						"Inner join la_ext_personlandmapping PL on LD.landid = PL.landid  "+
						"inner Join la_right_claimtype LC on LD.claimtypeid=LC.claimtypeid  "+
						"inner Join la_ext_applicationstatus la on la.applicationstatusid =  LD.applicationstatusid "+ 
						"inner Join la_ext_workflow lf on lf.workflowid =  LD.workflowstatusid "+
						"inner Join  la_party_person LP on PL.partyid = LP.personid " +
		                " inner join la_ext_transactiondetails TR on PL.transactionid = TR.transactionid "+ 
				         " where Pl.isactive=true and LD.workflowstatusid!=6 and LD.isactive=true and LD.projectnameid = " +project +" and " + strWhereCls + "  order by landid";*/
